    ...
    <div class="space-y-6 mx-auto p-6 max-w-7xl">
                ...
                <Table>
                    ...
                    <TableBody>
                        <TableRow v-for="match in matches" :key="match.id" class="hover:bg-gray-50">
                            ...
                            <TableCell>
                                <span class="bg-blue-100 px-2 py-1 rounded text-blue-800 text-xs font-semibold">
                                    Bisca {{ match.type }}
                                </span>
                            </TableCell>
                            <TableCell>
                                <div class="flex items-center gap-2">
                                    ...
                                    <Trophy v-if="match.winner_user_id === match.player1_user_id" class="w-4 h-4 text-yellow-500" />
                                </div>
                            </TableCell>
                            <TableCell>
                                <div class="flex items-center gap-2">
                                    ...
                                    <Trophy v-if="match.winner_user_id === match.player2_user_id" class="w-4 h-4 text-yellow-500" />
                                </div>
                            </TableCell>
                            <TableCell>
        ...
        <Card v-else class="shadow-lg">
            <CardContent class="py-12 text-center">
                <div class="flex justify-center items-center bg-gray-100 mx-auto mb-4 rounded-full w-16 h-16">
            ...
            <DialogContent class="max-w-3xl max-h-[80vh] overflow-y-auto">
                ...
                <div v-if="selectedMatch" class="space-y-6">
                    ...
                    <div class="bg-gray-50 p-4 rounded-lg">
                        <h4 class="font-semibold text-lg mb-3">Match Summary</h4>
                        <div class="gap-4 grid grid-cols-2">
                            ...
                            <div>
                                ...
                                <p class="font-medium">{{ selectedMatch.player1_marks }} - {{ selectedMatch.player2_marks }}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <!-- Players -->
                    <div class="gap-4 grid grid-cols-2">
                        <div class="border p-4 rounded-lg" :class="selectedMatch.winner_user_id === selectedMatch.player1_user_id ? 'bg-green-50 border-green-200' : ''">
                            <div class="flex items-center gap-3 mb-2">
                                <Avatar class="w-12 h-12">
                                    <AvatarImage :src="selectedMatch.player1?.avatar_url" />
                                    <AvatarFallback>{{ selectedMatch.player1?.nickname?.[0] || 'P1' }}</AvatarFallback>
                                </Avatar>
                                <div>
                                ...
                                <Trophy v-if="selectedMatch.winner_user_id === selectedMatch.player1_user_id" class="ml-auto w-6 h-6 text-yellow-500" />
                            </div>
                            <div class="space-y-1 text-sm">
                                <p>Points: <span class="font-semibold">{{ selectedMatch.player1_points }}</span></p>
                                <p>Marks: <span class="font-semibold">{{ selectedMatch.player1_marks }}</span></p>
                            </div>
                        </div>

                        <div class="border p-4 rounded-lg" :class="selectedMatch.winner_user_id === selectedMatch.player2_user_id ? 'bg-green-50 border-green-200' : ''">
                            <div class="flex items-center gap-3 mb-2">
                                <Avatar class="w-12 h-12">
                                    <AvatarImage :src="selectedMatch.player2?.avatar_url" />
                                    <AvatarFallback>{{ selectedMatch.player2?.nickname?.[0] || 'P2' }}</AvatarFallback>
                                </Avatar>
                                <div>
                                ...
                                <Trophy v-if="selectedMatch.winner_user_id === selectedMatch.player2_user_id" class="ml-auto w-6 h-6 text-yellow-500" />
                            </div>
                            <div class="space-y-1 text-sm">
                                <p>Points: <span class="font-semibold">{{ selectedMatch.player2_points }}</span></p>
                                <p>Marks: <span class="font-semibold">{{ selectedMatch.player2_marks }}</span></p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div v-if="selectedMatch.games && selectedMatch.games.length > 0">
                        <h4 class="font-semibold text-lg mb-3">Games Played ({{ selectedMatch.games.length }})</h4>
                        <div class="space-y-2">
                            <div v-for="(game, index) in selectedMatch.games" :key="game.id" 
                                class="flex justify-between items-center bg-white border p-3 rounded-lg">
                                <div class="flex items-center gap-3">
                                    ...
                                    <span class="text-sm">
                                        ...
                                        <span class="font-bold mx-2">{{ game.player1_points || 0 }} - {{ game.player2_points || 0 }}</span>
                                        {{ game.player2?.nickname || 'P2' }}
                                    </span>
                                </div>
                                <Trophy v-if="game.winner_user_id" class="w-4 h-4" 
                                    :class="game.winner_user_id === game.player1_user_id ? 'text-blue-500' : 'text-red-500'" 
                                />
...
const getCardDescription = () => {
    ...
    return viewMode.value === 'all' ? 'Complete match history for all players' : 'Your completed matches'
}

const getEmptyMessage = () => {
    if (!isAdmin.value) return "You haven't completed any matches yet."
    return viewMode.value === 'all' ? 'No completed matches in the system yet.' : "You haven't completed any matches yet."
}
...
const loadMatchHistory = async () => {
    ...
    error.value = ''
    try {
        const response = (isAdmin.value && viewMode.value === 'all')
...
const loadPage = (page) => {
    // For now just reload - could be enhanced with page parameter
    loadMatchHistory()
}
