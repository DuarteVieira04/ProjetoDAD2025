...
export function getOpenMatches() {
  return Array.from(matches.values()).filter((m) => m.status === 'pending' && !m.players.player2)
}
...
export async function endMatch(match, io, extra = {}) {
  ...
  const winnerKey = extra.winner
  const reason = extra.reason || 'marks_reached'

  console.log(
    `[MatchState] Ending match ${match.id} — Winner: ${winnerKey || 'draw'}, Reason: ${reason}`,
  )
...
async function saveMatchToDB(match, winnerKey) {
  ...
  try {
    ...
    const payload = {
      ...
      total_time: match.startTime ? Math.round((Date.now() - match.startTime) / 1000) : null,
    }

    console.log(`[MatchDB] Updating match ${match.id}...`, payload)
    const response = await axios.put(`${process.env.API_BASE_URL}/api/matches/${match.id}`, payload)
    console.log('[MatchDB] Updated:', response.data.id)
  } catch (error) {
    console.error('[MatchDB] Failed to save match:', error.response?.data || error.message)
  }
}
