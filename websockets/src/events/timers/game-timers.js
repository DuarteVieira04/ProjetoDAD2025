...
export async function endGame(game, io, extra = {}) {
  ...
  console.log(`[GameTimer] Ending single game ${game.id} — winner: ${winner || 'draw'}`)
...
async function saveGameToDB(game, winner) {
  ...
  try {
    ...
    await axios.put(`${process.env.API_BASE_URL}/api/games/${game.id}`, payload)
    console.log('[GameDB] Single game updated')
  } catch (error) {
    console.error('[GameDB] Failed to save single game:', error.response?.data || error.message)
  }
}
