...
function getWinningCards(hand, leadCard, trump) {
  const winningCards = [];
...
export function calculateBotMove(game, botPlayerKey) {
  ...
  if (game.currentTrick.length === 0) {
    ...
    if (sameSuitCards.length > 0) {
      // Must play same suit.
      possibleWinners = sameSuitCards.filter(
        (c) => getCardStrength(c.rank) > getCardStrength(trickCard.rank)
      );
    } else {
      // Can play any card.
    ...
    if (possibleWinners.length > 0) {
      ...
      possibleWinners.sort(
        (a, b) => getCardStrength(a.rank) - getCardStrength(b.rank)
      );
